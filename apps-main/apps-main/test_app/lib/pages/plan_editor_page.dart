...
class _PlanEditorPageState extends State<PlanEditorPage> {
  ...
  Future<void> _generatePlanByAI() async {
    // ... (AI 生成邏輯維持不變) ...
    if (_currentUser == null) return;
    ...
    try {
      final availableExercises = MockDataService.getAllExerciseNames();
      final aiService = AIService();
      final newPlans = await aiService.generateWorkoutPlan(_currentUser!, availableExercises);
      ...
      if (newPlans.isNotEmpty) {
        final confirm = await showDialog<bool>(
          context: context,
          builder: (ctx) => AlertDialog(
            title: const Row(children: [Icon(Icons.auto_awesome, color: Colors.purple), SizedBox(width: 8), Text('AI 課表生成完畢')]),
            content: Text('AI 已根據您的體重 (${_currentUser!.weight}kg) 為您規劃了專屬課表。\n\n共 ${newPlans.length} 個動作，是否套用？'),
            actions: [
              TextButton(child: const Text('取消'), onPressed: () => Navigator.pop(ctx, false)),
              ElevatedButton(
                child: const Text('套用'), 
        ...
        if (confirm == true) {
          ...
          for (var item in newPlans) {
            await DatabaseHelper.instance.insertPlanItem(item);
            setState(() {
               _daysSelected[item.dayOfWeek] = true;
            });
          }
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('AI 課表已成功套用！'), backgroundColor: Colors.purple),
            );
          }
        }
      } else {
         if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('AI 暫時無法生成，請稍後再試')));
      }
    } catch (e) {
      if (mounted) {
